...
const int BLKSIZE	= 4096; const int	NBLK	= 2047; const int	NULL = 0; const int	FNSIZE =	128;
const int	LBSIZE	= 4096; const int	ESIZE	= 256; const int	GBSIZE =	256; const int	NBRA =	5;
const int	EOF	= -1; const int KSIZE	= 9; const int	CBRA =	1; const int	CCHR =	2; const int	CDOT =	4;
const int	CCL =	6; const int	NCCL =	8; const int	CDOL =	10; const int	CEOF =	11; const int	CKET	= 12;
const int CBACK	= 14; const int	CCIRC	= 15; const int	STAR =	01; const int	READ =	0; const int WRITE =	1;
char	Q[]	= ""; char	T[]	= "TMP"; char	savedfile[FNSIZE]; char	file[FNSIZE]; char	linebuf[LBSIZE];
char	rhsbuf[LBSIZE/2]; char	expbuf[ESIZE+4]; char	genbuf[LBSIZE]; char	ibuff[BLKSIZE]; char	obuff[BLKSIZE];
char	WRERR[]	= "WRITE ERROR", *braslist[NBRA], *braelist[NBRA], tmpXXXXX[50] = "/tmp/eXXXXX";
char	*nextip, *linebp, *globp, *tfname, *loc1, *loc2;
int	peekc,	lastc, given, ninbuf, io, pflag, oflag, listf, listn, col, tline, ichanged, nleft, anymarks;
int	nbra, subnewa, subolda, fchange, wrapp, bpagesize = 20;
unsigned int	*addr1, *addr2, *dot, *dol, *zero;
long	count;
int	vflag	= 1; int	tfile	= -1; int	iblock	= -1; int	oblock	= -1; int	names[26];
unsigned nlall = 128;
jmp_buf	savej;
...
int main(int argc, char *argv[]) {
	char *p1, *p2;
	SIG_TYP oldintr;
	oldquit = signal(SIGQUIT, SIG_IGN); oldhup = signal(SIGHUP, SIG_IGN); oldintr = signal(SIGINT, SIG_IGN);
	...
	while (argc > 1 && **argv=='-') {
		...
		argv++; argc--;}
	if (oflag) {
		p1 = "/dev/stdout"; p2 = savedfile; while (*p2++ = *p1++){}
	}
	if (argc>1) {
...
void commands(void) {
	...
	switch(c) {
	case 'e': setnoaddr(); if (vflag && fchange) {fchange = 0; error(Q);}
	...
	caseread: if ((io = open(file, 0)) < 0) {lastc = '\n'; error(file);}
		setwide(); squeeze(0); ninbuf = 0; c = zero != dol; append(getfile, addr2); exfile(); fchange = c; continue;
	case 'z': getchr(); continue;
	case EOF: return;
	}
	error(Q);
	}
...
void print(void) {
	...
	do {
		if (listn) {count = a1-zero; putd(); putchr('\t');}
		puts(getline(*a1++));
	} while (a1 <= addr2);
	dot = addr2; listf = 0; listn = 0; pflag = 0;
}
unsigned int *address(void) {
	int sign, opcnt, nextopand, c; unsigned int *a, *b;
	nextopand = -1; sign = 1; opcnt = 0; a = dot;
	do {
		do c = getchr(); while (c==' ' || c=='\t');
		if ('0'<=c && c<='9') {
			peekc = c; if (!opcnt){a = zero;} a += sign*getnum();
		} else switch (c) {
		case '$': a = dol;
		case '.': if (opcnt) {error(Q);} break;
		case '\'': c = getchr(); if (opcnt || c<'a' || 'z'<c) {error(Q);} a = zero;
...
void exfile(void) {
	close(io); io = -1; if (vflag) {putd(); putchr('\n');}
}
void onintr(int n) {
...
void onhup(int n) {
	...
	if (dol > zero) {
		addr1 = zero+1; addr2 = dol; io = creat("ed.hup", 0600); if (io > 0) {putfile();}}
	fchange = 0; quit(0);
}
void error(char *s) {
	int c; wrapp = 0; listf = 0; listn = 0; putchr('?'); puts(s); count = 0; lseek(0, (long)0, 2);
...
int getfile(void) {
	int c; char *lp, *fp; lp = linebuf; fp = nextip;
	do {
		if (--ninbuf < 0) {
...
void putfile(void) {
	unsigned int *a1; int n, nib; char *fp, *lp; nib = BLKSIZE; fp = genbuf; a1 = addr1;
	do {
		lp = getline(*a1++);
		for (;;) {
			if (--nib < 0) {
...
void quit(int n) {
	if (vflag && fchange && dol!=zero) {fchange = 0; error(Q);} unlink(tfname); exit(0);
}
...
int putline(void) {
	char *bp, *lp; int nl; unsigned int tl; fchange = 1;
...
void init(void) {
	...
	subnewa = 0; anymarks = 0; iblock = -1; oblock = -1; ichanged = 0; close(creat(tfname, 0600));
...
int execute(unsigned int *addr) {
	...
	if (*p2==CCHR) {
		c = p2[1];
		do {if (*p1!=c) {continue;} if (advance(p1, p2)) {loc1 = p1; return(1);}} while (*p1++);
		return(0);}
...
int advance(char *lp, char *ep) {
	...
	star:
		do {
			lp--; if (advance(lp, ep)) {return(1);}
		} while (lp > curlp);
		return(0);
	default: error(Q);}
}
int backref(int i, char *lp) {
	char *bp; bp = braslist[i]; while (*bp++ == *lp++) {if (bp >= braelist[i]) {return(1);}} return(0);
}
...
void putd(void) {
	int r; r = count%10; count /= 10;
	if (count) {putd();} putchr(r + '0');
}
void puts(char *sp) {
	col = 0; while (*sp) {putchr(*sp++);} putchr('\n');
}
char	line[70]; char	*linp	= line;
void putchr(int ac) {
	char *lp; int c; lp = linp; c = ac;
	if (listf) {
		if (c=='\n') {
			...
			if (c=='\b' || c=='\t' || c=='\\') {
				...
				*lp++ = '\\'; *lp++ =  (c>>6)    +'0'; *lp++ = ((c>>3)&07)+'0'; c     = ( c    &07)+'0';
				col += 3;}}
	}
	*lp++ = c;
